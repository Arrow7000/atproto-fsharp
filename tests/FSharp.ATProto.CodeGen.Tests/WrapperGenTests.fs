...
let wrapperTests =
    testList
        "wrapper generation"
        [ testCase "query with params generates query function"
          <| fun () ->
          ...
          testCase "query without params generates query function with just agent"
          <| fun () ->
          ...
          testCase "procedure with input+output generates call function"
          <| fun () ->
          ...
          testCase "procedure with input only generates call function with unit result"
          <| fun () ->
              let result =
                  FSharp.ATProto.CodeGen.NamespaceGen.generateAll [ procedureInputOnlyDoc ]
          ...
          testCase "query with ref output generates query function with Output type alias"
          <| fun () ->
              let result =
                  FSharp.ATProto.CodeGen.NamespaceGen.generateAll [ queryWithRefOutputDoc ]
          ...
          testCase "procedure with ref output generates call function with Output type alias"
          <| fun () ->
              let result =
                  FSharp.ATProto.CodeGen.NamespaceGen.generateAll [ procedureWithRefOutputDoc ]
          ...
          testCase "record doc does not get a wrapper function"
          <| fun () ->
          ...
          testCase "generated code includes open statements"
          <| fun () ->
              let result = FSharp.ATProto.CodeGen.NamespaceGen.generateAll [ queryWithParamsDoc ]
              let (_, content) = result.[0]
          ...
          testCase "wrapper is placed after TypeId line"
          <| fun () ->
              let result = FSharp.ATProto.CodeGen.NamespaceGen.generateAll [ queryWithParamsDoc ]
              let (_, content) = result.[0]
          ...
          testCase "multiple docs generate wrappers for queries and procedures"
          <| fun () ->
              let result =
                  FSharp.ATProto.CodeGen.NamespaceGen.generateAll [ queryWithParamsDoc; procedureWithIODoc ]

              let (_, content) = result.[0]
              Expect.stringContains content "let query" "has query function"
              Expect.stringContains content "let call" "has call function"

          testCase "typed string formats emit Syntax types instead of string"
          <| fun () ->
              let result =
                  FSharp.ATProto.CodeGen.NamespaceGen.generateAll [ typedStringFormatsDoc ]
          ...
          testCase "inline union generates DU type"
          <| fun () ->
              let result =
                  FSharp.ATProto.CodeGen.NamespaceGen.generateAll [ recordWithInlineUnionDoc ]
              ...
              Expect.isFalse
                  (content.Contains "Embed: System.Text.Json.JsonElement")
                  "embed is not System.Text.Json.JsonElement"

          testCase "closed inline union has no Unknown case"
          <| fun () ->
              let result =
                  FSharp.ATProto.CodeGen.NamespaceGen.generateAll [ recordWithClosedUnionDoc ]
          ...
          testCase "array of union generates DU with list type"
          <| fun () ->
              let result =
                  FSharp.ATProto.CodeGen.NamespaceGen.generateAll [ recordWithArrayUnionDoc ]
          ...
          testCase "query output with inline union generates DU type"
          <| fun () ->
              let result =
                  FSharp.ATProto.CodeGen.NamespaceGen.generateAll [ queryWithInlineUnionOutputDoc ]
