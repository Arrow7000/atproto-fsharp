                              ...
                              { Description = None
                                ...
                                Schema =
                                  Some
                                      (LexType.Object
                                          { Description = None
                                            Properties = Map.ofList [ "items", LexType.Array { Description = None; Items = LexType.String emptyStringConstraints; MinLength = None; MaxLength = None } ]
                                            Required = [ "items" ]
                                            Nullable = [] }) }
                      else
                          None
...
let groupByNamespaceTests =
    testList "namespace grouping" [
        testCase "groups docs by 3-segment namespace" <| fun () ->
            let doc1 = mkRecordDoc "app.bsky.feed.post" [ "text", LexType.String emptyStringConstraints ] [ "text" ]
            let doc2 = mkRecordDoc "app.bsky.feed.like" [ "subject", LexType.String emptyStringConstraints ] [ "subject" ]
            let doc3 = mkRecordDoc "com.atproto.repo.strongRef" [ "uri", LexType.String emptyStringConstraints ] [ "uri" ]
        ...
        testCase "single doc produces single namespace" <| fun () ->
            let doc = mkRecordDoc "app.bsky.actor.profile" [ "name", LexType.String emptyStringConstraints ] [ "name" ]
...
let topologicalSortTests =
    testList "topological sort" [
        testCase "dependencies come first" <| fun () ->
            let deps =
                Map.ofList [
        ...
        testCase "no dependencies returns all nodes" <| fun () ->
            let deps =
                Map.ofList [
                    "A", Set.empty
                    "B", Set.empty
        ...
        testCase "handles cycles gracefully" <| fun () ->
            let deps =
                Map.ofList [
                    "A", Set.ofList [ "B" ]
                    "B", Set.ofList [ "A" ]
        ...
        testCase "empty map returns empty list" <| fun () ->
            let order = topologicalSort Map.empty
            Expect.equal (List.length order) 0 "empty"

        testCase "includes dependency-only namespaces" <| fun () ->
...
let collectDependenciesTests =
    testList "collectDependencies" [
        testCase "collects cross-namespace ref" <| fun () ->
            let doc =
        ...
        testCase "same-namespace ref is not a dependency" <| fun () ->
            let doc =
                mkRecordDoc "app.bsky.feed.post"
                    [ "reply", LexType.Ref { Description = None; Ref = "app.bsky.feed.post#replyRef" } ]
                    [ "reply" ]

            let deps = collectDependencies "AppBskyFeed" [ doc ]
            Expect.isFalse (Set.contains "AppBskyFeed" deps) "no self-dependency"

        testCase "collects union ref dependencies" <| fun () ->
            let doc =
                { Lexicon = 1
...
let generateAllTests =
    testList "generateAll" [
        ...
        testCase "generates token as literal constant" <| fun () ->
            let doc =
                { Lexicon = 1
...
let realLexiconTests =
    testList "real lexicons" [
        testCase "all real lexicon files parse and group into namespaces" <| fun () ->
