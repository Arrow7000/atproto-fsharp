...
let generateRecordTests =
    testList
        "generateRecord"
        [ testCase "simple record with required string fields"
          <| fun () ->
              let obj : LexObject =
                  { Description = None
                    Properties =
          ...
          testCase "record with optional field"
          <| fun () ->
              let obj : LexObject =
                  { Description = None
                    Properties =
          ...
          testCase "record with description generates XML doc"
          <| fun () ->
              let obj : LexObject =
                  { Description = None
                    Properties = Map.ofList [ "name", LexType.String emptyStringConstraints ]
                    Required = [ "name" ]
                    Nullable = [] }
          ...
          testCase "record with nullable field"
          <| fun () ->
              let obj : LexObject =
                  { Description = None
                    Properties = Map.ofList [ "value", LexType.String emptyStringConstraints ]
                    Required = [ "value" ]
                    Nullable = [ "value" ] }
          ...
          testCase "record field has JsonPropertyName attribute with original name"
          <| fun () ->
              let obj : LexObject =
                  { Description = None
                    Properties = Map.ofList [ "createdAt", LexType.String emptyStringConstraints ]
                    Required = [ "createdAt" ]
                    Nullable = [] }
          ...
          testCase "record with boolean field"
          <| fun () ->
              let obj : LexObject =
                  { Description = None
                    Properties = Map.ofList [ "active", LexType.Boolean emptyBooleanConstraints ]
                    Required = [ "active" ]
                    Nullable = [] }
          ...
          testCase "record with integer field"
          <| fun () ->
              let obj : LexObject =
                  { Description = None
                    Properties = Map.ofList [ "count", LexType.Integer emptyIntegerConstraints ]
                    Required = [ "count" ]
                    Nullable = [] }
          ...
          testCase "record with bytes field"
          <| fun () ->
              let obj : LexObject =
                  { Description = None
                    Properties = Map.ofList [ "data", LexType.Bytes emptyBytesConstraints ]
                    Required = [ "data" ]
                    Nullable = [] }
          ...
          testCase "record with array of strings field"
          <| fun () ->
              let obj : LexObject =
                  { Description = None
                    Properties =
          ...
          testCase "record with ref field"
          <| fun () ->
              let obj : LexObject =
                  { Description = None
                    Properties =
          ...
          testCase "record with optional ref field is option"
          <| fun () ->
              let obj : LexObject =
                  { Description = None
                    Properties =
          ...
          testCase "record fields are in deterministic order"
          <| fun () ->
              let obj : LexObject =
                  { Description = None
                    Properties =
          ...
          testCase "record with mixed required and optional fields"
          <| fun () ->
              let obj : LexObject =
                  { Description = None
                    Properties =
          ...
          testCase "record with no description does not have XML doc"
          <| fun () ->
              let obj : LexObject =
                  { Description = None
...
let generateTokenTests =
    testList
        "generateToken"
        [ testCase "token generates literal constant"
          <| fun () ->
          ...
          testCase "token generates let binding"
          <| fun () ->
              let token : LexToken = { Description = None }
              let source = generateToken "app.bsky.feed.defs" "feedViewPost" token
...
let generateKnownValuesTests =
    testList
        "generateKnownValues"
        [ testCase "knownValues generates module with constants"
          <| fun () ->
              let source =
                  generateKnownValues "sort" [ "app.bsky.feed.defs#sortHot"; "app.bsky.feed.defs#sortNew" ]
          ...
          testCase "knownValues constants have Literal attribute"
          <| fun () ->
              let source = generateKnownValues "sort" [ "app.bsky.feed.defs#sortHot" ]
              Expect.stringContains source "[<Literal>]" "should have Literal attribute"

          testCase "knownValues constants have full value string"
          <| fun () ->
              let source = generateKnownValues "sort" [ "app.bsky.feed.defs#sortHot" ]
              Expect.stringContains source "\"app.bsky.feed.defs#sortHot\"" "should have full value string"

          testCase "knownValues with dot-only values (no fragment)"
          <| fun () ->
          ...
          testCase "knownValues extracts name after hash"
          <| fun () ->
          ...
          testCase "knownValues module name is PascalCased"
          <| fun () ->
          ...
          testCase "knownValues with multiple values generates multiple lets"
          <| fun () ->
              let source =
                  generateKnownValues
                      "sort"
...
let unionCaseNameTests =
    testList
        "unionCaseName"
        [ testCase "unionCaseName extracts correct names"
          <| fun () ->
...
let generateUnionTests =
    testList
        "generateUnion"
        [ testCase "open union generates DU with Unknown case"
          <| fun () ->
              let union : LexUnion =
          ...
          testCase "closed union has no Unknown case"
          <| fun () ->
              let union : LexUnion =
                  { Description = None
          ...
          testCase "union cases reference correct types"
          <| fun () ->
              let union : LexUnion =
                  { Description = None
          ...
          testCase "union has JsonName attributes with tag values"
          <| fun () ->
              let union : LexUnion =
                  { Description = None
          ...
          testCase "union has JsonFSharpConverter attribute"
          <| fun () ->
              let union : LexUnion =
                  { Description = None
          ...
          testCase "union with description generates XML doc"
          <| fun () ->
              let union : LexUnion =
          ...
          testCase "union without description has no XML doc"
          <| fun () ->
              let union : LexUnion =
                  { Description = None
                    Refs = [ "app.bsky.embed.images" ]
          ...
          testCase "union #main ref strips #main from tag value"
          <| fun () ->
              let union : LexUnion =
                  { Description = None
          ...
          testCase "union deduplicates case names"
          <| fun () ->
...
let generateKnownValueDUTests =
    testList
        "generateKnownValueDU"
        [ testCase "generates DU with JsonName attributes"
          <| fun () ->
              let widget = generateKnownValueDU "Reason" [ "like"; "repost"; "follow" ]
              let source = Oak () { AnonymousModule () { widget } } |> Gen.mkOak |> Gen.run
          ...
          testCase "generates JsonConverter attribute"
          <| fun () ->
          ...
          testCase "handles hash-fragment known values"
          <| fun () ->
              let widget =
                  generateKnownValueDU "Sort" [ "app.bsky.feed.defs#sortHot"; "app.bsky.feed.defs#sortNew" ]
          ...
          testCase "deduplicates case names"
          <| fun () ->
          ...
          testCase "PascalCases simple known values"
          <| fun () ->
