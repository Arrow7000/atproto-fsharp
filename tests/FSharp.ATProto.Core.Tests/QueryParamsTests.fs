...
let tests =
    testList
        "QueryParams"
        [ testCase "serializes string field"
          <| fun () ->
          ...
          testCase "serializes Did field via ToString"
          <| fun () ->
              let did =
                  Did.parse "did:plc:z72i7hdynmk6r22z27h6tvur" |> Result.defaultWith failwith
          ...
          testCase "serializes Handle field via ToString"
          <| fun () ->
          ...
          testCase "serializes AtUri field via ToString"
          <| fun () ->
              let uri =
              ...
              Expect.equal
                  result
                  "?uri=at%3A%2F%2Fdid%3Aplc%3Az72i7hdynmk6r22z27h6tvur%2Fapp.bsky.feed.post%2F3k2la3b"
                  "AtUri field"

          testCase "serializes Cid field via ToString"
          <| fun () ->
          ...
          testCase "serializes Nsid field via ToString"
          <| fun () ->
          ...
          testCase "serializes optional Did field"
          <| fun () ->
              let did =
                  Did.parse "did:plc:z72i7hdynmk6r22z27h6tvur" |> Result.defaultWith failwith
          ...
          testCase "serializes known-value DU fieldless case"
          <| fun () ->
          ...
          testCase "serializes known-value DU Unknown case"
          <| fun () ->
              let result =
                  QueryParams.toQueryString { KnownValueParams.Reason = TestReason.Unknown "custom-reason" }

              Expect.equal result "?reason=custom-reason" "Unknown case extracts string"

          testCase "serializes optional known-value DU"
          <| fun () ->
              let result =
                  QueryParams.toQueryString { OptionalKnownValueParams.Reason = Some TestReason.Repost }

              Expect.equal result "?reason=repost" "optional known value Some"

          testCase "serializes known-value DU list"
          <| fun () ->
              let result =
                  QueryParams.toQueryString { KnownValueListParams.Reasons = [ TestReason.Like; TestReason.Repost ] }
