...
let tests =
    testList "TaskResult CE" [
        ...
        testCase "bind Error short-circuits" <| fun () ->
        ...
        testCase "returnFrom passes through" <| fun () ->
            let result =
                (taskResult {
                    return! Task.FromResult(Ok "hello")
                }).GetAwaiter().GetResult()
            Expect.equal result (Ok "hello") "should pass through"

        testCase "returnFrom Error passes through" <| fun () ->
            let result =
                (taskResult {
                    return! Task.FromResult(Error "fail")
                }).GetAwaiter().GetResult()
            Expect.equal result (Error "fail") "should pass through Error"

        testCase "nested taskResult works" <| fun () ->
        ...
        testCase "try-with catches exceptions" <| fun () ->
            let result =
                (taskResult {
                    try
                        failwith "test error"
        ...
        testCase "using disposes resource" <| fun () ->
            let disposed = ref false
            let result =
                (taskResult {
                    use _ = { new System.IDisposable with member _.Dispose() = disposed.Value <- true }
                    return 42
                }).GetAwaiter().GetResult()
