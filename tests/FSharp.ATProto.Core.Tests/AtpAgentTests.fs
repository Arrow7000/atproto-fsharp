...
let loginTests =
    testList
        "AtpAgent.login"
        [ testCase "successful login stores session"
          <| fun () ->
              let agent =
                  makeAgent (fun req ->
          ...
          testCase "failed login returns error"
          <| fun () ->
              let agent =
                  makeAgent (fun _ ->
                      TestHelpers.jsonResponse
              ...
              let result =
                  AtpAgent.login "bad" "bad" agent |> Async.AwaitTask |> Async.RunSynchronously
...
let refreshTests =
    testList
        "Xrpc auto-refresh"
        [ testCase "retries with new token on 401 ExpiredToken"
          <| fun () ->
              ...
              let agent =
                  makeAgent (fun req ->
                      ...
                      if uri.Contains ("refreshSession") then
                          ...
                          TestHelpers.jsonResponse HttpStatusCode.OK {| displayName = "Alice" |})

              agent.Session <-
                  Some
                      { AccessJwt = "old"
                        RefreshJwt = "refresh1"
                        Did = parseDid "did:plc:alice"
          ...
          testCase "does not refresh when no session exists"
          <| fun () ->
              let agent =
                  makeAgent (fun _ ->
                      TestHelpers.jsonResponse
          ...
          testCase "returns refresh error if refresh itself fails"
          <| fun () ->
              let agent =
                  makeAgent (fun req ->
                      let uri = string req.RequestUri
              ...
              agent.Session <-
                  Some
                      { AccessJwt = "old"
                        RefreshJwt = "oldref"
                        Did = parseDid "did:plc:x"
                        Handle = parseHandle "x.test" }
