...
let withChatProxyTests =
    testList
        ...
        [ testCase "adds atproto-proxy header"
          <| fun () ->
              let agent =
                  makeAgent (fun _ -> TestHelpers.jsonResponse HttpStatusCode.OK {| ok = true |})

              let chatAgent = AtpAgent.withChatProxy agent

              Expect.contains
                  chatAgent.ExtraHeaders
                  ("atproto-proxy", "did:web:api.bsky.chat#bsky_chat")
                  "proxy header present"

          testCase "preserves existing extra headers"
          <| fun () ->
              let agent =
                  makeAgent (fun _ -> TestHelpers.jsonResponse HttpStatusCode.OK {| ok = true |})
          ...
          testCase "new agent has empty extra headers"
          <| fun () ->
              let agent =
                  makeAgent (fun _ -> TestHelpers.jsonResponse HttpStatusCode.OK {| ok = true |})
...
let extraHeadersInXrpcTests =
    testList
        "XRPC extra headers"
        [ testCase "query includes extra headers in request"
          <| fun () ->
          ...
          testCase "procedure includes extra headers in request"
          <| fun () ->
              let mutable capturedRequest : HttpRequestMessage option = None
          ...
          testCase "procedureVoid includes extra headers in request"
          <| fun () ->
              let mutable capturedRequest : HttpRequestMessage option = None
          ...
          testCase "queryNoParams includes extra headers in request"
          <| fun () ->
              let mutable capturedRequest : HttpRequestMessage option = None
          ...
          testCase "multiple extra headers are all applied"
          <| fun () ->
              let mutable capturedRequest : HttpRequestMessage option = None
              ...
              let agentWithHeaders =
                  { agent with
                      ExtraHeaders = [ ("atproto-proxy", "did:web:api.bsky.chat#bsky_chat"); ("x-custom", "hello") ] }
