...
let tests =
    testList
        ...
        [ testList
              "encode"
              [ testCase "empty" <| fun () -> Expect.equal (Base32.encode [||]) "" "empty"
          ...
          testList
              "decode"
              [ testCase "empty" <| fun () -> Expect.equal (Base32.decode "") [||] "empty"
                testCase "single byte"
                <| fun () -> Expect.equal (Base32.decode "my") [| 0x66uy |] "decode 'my'"
                testCase "two bytes"
                <| fun () -> Expect.equal (Base32.decode "mzxq") [| 0x66uy; 0x6Fuy |] "decode 'mzxq'"
                testCase "three bytes"
                <| fun () -> Expect.equal (Base32.decode "mzxw6") [| 0x66uy; 0x6Fuy; 0x6Fuy |] "decode 'mzxw6'" ]
          testList
              "roundtrip"
              [ testCase "various lengths"
                <| fun () ->
