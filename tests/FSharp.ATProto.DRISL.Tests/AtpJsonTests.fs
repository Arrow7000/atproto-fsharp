    ...
    testList
        ...
        [ testList
              ...
              [ testCase "simple object"
                ...
                testCase "integer-like float accepted"
                <| fun () ->
                    ...
                    match value with
                    | AtpValue.Object m -> Expect.equal (Map.find "a" m) (AtpValue.Integer 123L) "123.0 -> Integer 123"
                    | _ -> failwith "expected object"

                testCase "rejects non-integer float"
                <| fun () ->
                    let result = AtpJson.fromJson (parse """{"a": 123.456}""")
                    Expect.isError result "should reject float"

                testCase "rejects bare string at top level"
                <| fun () ->
                    let result = AtpJson.fromJson (parse "\"blah\"")
                    Expect.isError result "top-level must be object"

                testCase "rejects bare number at top level"
                <| fun () ->
          ...
          testList
              ...
              [ testCase "valid link"
                <| fun () ->
                    let json =
                        """{"a": {"$link": "bafyreidfayvfuwqa7qlnopdjiqrxzs6blmoeu4rujcjtnci5beludirz2a"}}"""
                ...
                testCase "rejects link with wrong type"
                <| fun () ->
                    let result = AtpJson.fromJson (parse """{"a": {"$link": 1234}}""")
                    Expect.isError result "link value must be string"

                testCase "rejects link with extra fields"
                <| fun () ->
                    let result =
                        AtpJson.fromJson (
                            parse
                                """{"a": {"$link": "bafyreidfayvfuwqa7qlnopdjiqrxzs6blmoeu4rujcjtnci5beludirz2a", "other": "blah"}}"""
                        )
          ...
          testList
              ...
              [ testCase "valid blob"
                <| fun () ->
                    let json =
                        """{"blb": {"$type": "blob", "ref": {"$link": "bafkreiccldh766hwcnuxnf2wh6jgzepf2nlu2lvcllt63eww5p6chi4ity"}, "mimeType": "image/jpeg", "size": 10000}}"""

                    let result = AtpJson.fromJson (parse json)
                    Expect.isOk result "valid blob"

                testCase "rejects blob with string size"
                <| fun () ->
                    let json =
                        """{"blb": {"$type": "blob", "ref": {"$link": "bafkreiccldh766hwcnuxnf2wh6jgzepf2nlu2lvcllt63eww5p6chi4ity"}, "mimeType": "image/jpeg", "size": "10000"}}"""
                ...
                testCase "rejects blob with missing ref"
                <| fun () ->
