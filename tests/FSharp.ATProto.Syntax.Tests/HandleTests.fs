...
let tests =
    testList
        "Handle"
        [ testList
              "valid handles parse successfully"
              [ for h in validItems do
                    testCase h
                    <| fun () -> Expect.isOk (Handle.parse h) (sprintf "should parse: %s" h) ]
          testList
              "invalid handles are rejected"
              [ for h in invalidItems do
                    testCase h
                    <| fun () -> Expect.isError (Handle.parse h) (sprintf "should reject: %s" h) ]
          testList
              "roundtrip"
              [ for h in validItems do
                    testCase (sprintf "roundtrip %s" h)
                    <| fun () ->
