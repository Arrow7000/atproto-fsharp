...
let tests =
    testList "Uri" [
        testList "valid URIs parse successfully" [
            for c in validItems do
                testCase c <| fun () ->
                    Expect.isOk (Uri.parse c) (sprintf "should parse: %s" c)
        ]
        testList "invalid URIs are rejected" [
            for c in invalidItems do
                testCase c <| fun () ->
                    Expect.isError (Uri.parse c) (sprintf "should reject: %s" c)
        ]
        testList "roundtrip" [
            for c in validItems do
