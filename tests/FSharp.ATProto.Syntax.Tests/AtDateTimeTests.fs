...
let tests =
    testList "AtDateTime" [
        testList "valid datetimes parse successfully" [
            for c in validItems do
                testCase c <| fun () ->
                    Expect.isOk (AtDateTime.parse c) (sprintf "should parse: %s" c)
        ]
        testList "invalid datetimes are rejected" [
            for c in invalidItems do
                testCase c <| fun () ->
                    Expect.isError (AtDateTime.parse c) (sprintf "should reject: %s" c)
        ]
        testList "roundtrip" [
            for c in validItems do
