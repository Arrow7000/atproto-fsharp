...
let rkeyCharset = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789._~:-"
...
let genRecordKeyString =
    gen {
        ...
        if s = "." || s = ".." then return "valid-key"
        else return s
    }

let genValidRecordKey = genRecordKeyString |> Gen.map (RecordKey.parse >> Result.defaultWith failwith)
