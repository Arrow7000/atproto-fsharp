...
let private countGraphemes (s : string) = Globalization.StringInfo(s).LengthInTextElements
...
let private resolveRef (catalog : Map<string, LexiconDoc>) (refStr : string) : Result<LexDef, string> =
...
let private validateFormat (format : LexStringFormat) (value : string) : Result<unit, string> =
    let mapResult r = r |> Result.map (fun _ -> ())
...
let rec private validateType
    ...
    | LexType.Integer constraints ->
        if element.ValueKind <> JsonValueKind.Number then
            ...
            let checkConst () =
                ...
                | Some c when value <> c -> Error (sprintf "Integer value %d does not match const %d" value c)
                | _ -> Ok ()
            ...
            let checkMin () =
                ...
                | Some min when value < min -> Error (sprintf "Integer value %d below minimum %d" value min)
                | _ -> Ok ()

            let checkMax () =
                match constraints.Maximum with
                | Some max when value > max -> Error (sprintf "Integer value %d above maximum %d" value max)
                | _ -> Ok ()
    ...
    | LexType.String constraints ->
        if element.ValueKind <> JsonValueKind.String then
            ...
            let checkConst () =
                match constraints.Const with
                | Some c when value <> c -> Error (sprintf "String value '%s' does not match const '%s'" value c)
                | _ -> Ok ()
                ...
                if bytesVal.ValueKind <> JsonValueKind.String then
                    ...
                    | _ ->
                        try
                            ...
                            match constraints.MinLength with
                            | Some min when len < min -> Error (sprintf "Bytes length %d below minLength %d" len min)
                            | _ ->
                                match constraints.MaxLength with
    ...
    | LexType.Blob constraints -> validateBlob catalog constraints element
    ...
    | LexType.Array arr ->
        if element.ValueKind <> JsonValueKind.Array then
            ...
            let checkMinLen () =
                ...
                | Some min when len < min -> Error (sprintf "Array length %d below minLength %d" len min)
                | _ -> Ok ()

            let checkMaxLen () =
                match arr.MaxLength with
                | Some max when len > max -> Error (sprintf "Array length %d above maxLength %d" len max)
                | _ -> Ok ()
            ...
            let checkElements () =
                ...
                for item in element.EnumerateArray () do
                    if err.IsNone then
                        ...
                        | Ok () -> ()
                        | Error e -> err <- Some (sprintf "Array element [%d]: %s" idx e)
    ...
    | LexType.Object obj -> validateObject catalog obj element
        ...
        | Ok def ->
            ...
            | LexDef.Token _ ->
                if element.ValueKind = JsonValueKind.String then
                    ...
                    Error (sprintf "Expected string for token ref, got %A" element.ValueKind)
            ...
            | true, typeVal ->
                if typeVal.ValueKind <> JsonValueKind.String then
                    ...
                    if union.Closed && not isInRefs then
                        ...
                        | Ok def ->
                            match def with
                            | LexDef.DefType (LexType.Object obj) -> validateObject catalog obj element
                            | LexDef.DefType lexType -> validateType catalog lexType element
                            | LexDef.Record record -> validateObject catalog record.Record element
                            | _ -> Ok ()
                    else
        ...
        let checkRequired () =
            ...
            for reqProp in obj.Required do
                if err.IsNone then
                    ...
                    | false, _ -> err <- Some (sprintf "Required property '%s' is missing" reqProp)
                    | true, propVal ->
                        if
                            propVal.ValueKind = JsonValueKind.Null
                            && not (List.contains reqProp obj.Nullable)
                        then
                    ...
                    if propName <> "$type" then
                        ...
                        | Some propSchema ->
                            if kv.Value.ValueKind = JsonValueKind.Null then
                                ...
                                | Ok () -> ()
                                | Error e -> propErr <- Some (sprintf "Property '%s': %s" propName e)
                        | None -> ()
    ...
    | true, typeVal ->
        if typeVal.ValueKind <> JsonValueKind.String then
            ...
            | Some doc ->
                ...
                | Some def ->
                    match def with
                    | LexDef.Record record -> validateObject catalog record.Record data
