...
let rec lexTypeToFSharpType (currentNamespace : string) (lexType : LexType) : string =
    ...
    | Ref r ->
        let (_targetNamespace, qualifiedName) =
            Naming.refToQualifiedType currentNamespace r.Ref
...
let rec collectNamespaceDeps (currentNamespace : string) (lexType : LexType) : Set<string> =
    match lexType with
    | Ref r ->
        let (targetNamespace, _qualifiedName) =
            Naming.refToQualifiedType currentNamespace r.Ref
        ...
        if targetNamespace <> currentNamespace then
            ...
            Set.empty
    | Array arr -> collectNamespaceDeps currentNamespace arr.Items
    | Union u ->
        u.Refs
        |> List.map (fun ref ->
