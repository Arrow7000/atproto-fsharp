...
let private cleanIdentifier (raw: string) : string =
    ...
    |> fun s ->
        ...
        if System.String.IsNullOrEmpty(trimmed) then "Value" + s else trimmed
...
let generateKnownValueDU (typeName: string) (values: string list) =
    let cases =
        ...
        |> List.map (fun (caseName, v) ->
            if caseName = "Unknown" then ("UnknownValue", v) else (caseName, v))
        |> List.fold (fun (acc, seen: Map<string, int>) (caseName, origValue) ->
            ...
            | None ->
                ((caseName, origValue) :: acc, Map.add caseName 1 seen)
        ) ([], Map.empty)
        |> fst
        |> List.rev
        ...
        for (caseName, origValue) in cases do
            UnionCase(caseName)
                .attribute(
                    Attribute(
                        "JsonName",
                        ParenExpr(ConstantExpr(Ast.String(origValue)))))
        UnionCase("Unknown", "string")
    })
        .attribute(
            Attribute(
                sprintf "JsonConverter(typeof<FSharp.ATProto.Core.KnownValueConverter<%s>>)" typeName))
