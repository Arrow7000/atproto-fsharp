...
module QueryParams =
    ...
    let private isOptionType (t : Type) = t.IsGenericType && t.GetGenericTypeDefinition () = typedefof<option<_>>

    let private isListType (t : Type) = t.IsGenericType && t.GetGenericTypeDefinition () = typedefof<list<_>>
    ...
    let toQueryString<'T> (record : 'T) : string =
        ...
        if Array.isEmpty pairs then
            ...
            "?" + (pairs |> Array.map (fun (k, v) -> $"{k}={v}") |> String.concat "&")
